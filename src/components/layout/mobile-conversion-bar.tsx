...
export function MobileConversionBar() {
  ...
  return <nav aria-label="Quick booking actions" className="fixed inset-x-0 bottom-0 z-40 max-w-full border-t border-line bg-white/95 px-2 pt-2 pb-[calc(.5rem+env(safe-area-inset-bottom))] shadow-[0_-8px_30px_rgba(7,26,46,.1)] backdrop-blur md:hidden"><div className="flex min-w-0 gap-1">
    ...
    {whatsappHref ? <a href={whatsappHref} className="flex min-h-14 min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-xl text-[11px] font-extrabold text-navy-900"><MessageCircle size={19} />WhatsApp</a> : <span aria-disabled="true" className="flex min-h-14 min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-xl bg-[#f5f7f8] text-[11px] font-extrabold text-muted"><MessageCircle size={19} />WhatsApp</span>}
