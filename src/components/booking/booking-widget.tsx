...
export function BookingWidget() {
  ...
  return <div id="book" className="card min-w-0 scroll-mt-28 overflow-visible border-white/70 bg-[#fffefa] shadow-[0_24px_70px_rgba(0,9,20,.28)]">
    ...
    <form onSubmit={handleSubmit(submit)} noValidate className="bg-white p-4 sm:p-6">
      <fieldset className="min-w-0"><legend className="sr-only">Choose trip type</legend><div className="grid min-w-0 grid-cols-3 gap-1 rounded-xl bg-[#f1f4f6] p-1">{tabs.map(({ id, label, icon: Icon }) => <label key={id} className={cn("flex min-h-12 min-w-0 cursor-pointer flex-col items-center justify-center gap-0.5 rounded-lg px-0.5 text-center text-[11px] leading-tight font-extrabold transition-colors min-[360px]:flex-row min-[360px]:gap-1 min-[360px]:text-xs sm:text-sm", tripType === id ? "bg-navy-900 text-white shadow-sm" : "text-muted hover:bg-white")}><input type="radio" value={id} className="sr-only" {...register("tripType")} /><Icon size={16} className="shrink-0" />{label}</label>)}</div></fieldset>
